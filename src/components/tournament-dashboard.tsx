  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8">
        ...
        <div className="relative flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-3 sm:gap-4">
            {summary.map((item) => (
              <div
                key={item.label}
                className="min-w-24 rounded-[1.5rem] border border-white/70 bg-[var(--surface-strong)] px-4 py-5 text-center shadow-[0_12px_30px_rgba(16,185,129,0.12)]"
              >
                <p className="text-xs font-semibold uppercase tracking-[0.25em] text-emerald-500">
                  {item.label}
                </p>
                <p className="mt-2 text-3xl font-bold text-emerald-950">{item.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="grid gap-8 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[2rem] border border-white/60 bg-[var(--surface)] p-6 shadow-[0_18px_60px_rgba(22,101,52,0.12)] backdrop-blur sm:p-8">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-500">
                ผลการแข่งขัน
              </p>
              <h2 className="font-serif text-3xl text-emerald-950">กระดานแข่งขัน</h2>
            </div>
          ...
          {data || resultDocuments.length ? (
            <div className="grid gap-4">
                ...
                ? data.matches.map((match) => (
                    ...
                    >
                      <div className="mb-4 flex items-center justify-between gap-4">
                        ...
                        <p className="rounded-full bg-emerald-600 px-3 py-1 text-sm font-semibold text-white">
                          {match.result}
                        </p>
                      </div>
                      <div className="grid gap-3 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
                        <div>
                          <p className="text-lg font-semibold text-emerald-950">{match.leftName}</p>
                          <p className="text-sm text-emerald-700/70">คะแนนก่อนแข่ง {match.leftScore}</p>
                        </div>
                        <div className="justify-self-center rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700">
                          พบกับ
                        </div>
                        <div className="sm:text-right">
                          <p className="text-lg font-semibold text-emerald-950">{match.rightName}</p>
                          <p className="text-sm text-emerald-700/70">คะแนนก่อนแข่ง {match.rightScore}</p>
                        </div>
                      </div>
                      <p className="mt-4 text-sm text-emerald-900/75">
                        ผู้ชนะ: <span className="font-semibold text-emerald-950">{match.winner}</span>
                      </p>
                    </article>
                  ))
                : null}
              {resultDocuments.map((document) => (
                ...
                >
                  ...
                  <pre className="overflow-x-auto whitespace-pre-wrap rounded-[1.25rem] bg-emerald-50/80 p-4 text-sm leading-7 text-emerald-950">
            ...
            <div className="rounded-[1.75rem] border border-dashed border-emerald-200 bg-white/60 px-6 py-14 text-center text-emerald-700/75">
        ...
        <div className="flex flex-col gap-8">
            ...
            {visiblePlayers.length ? (
              <div className="mt-6 grid gap-3">
                {visiblePlayers.map((player, index) => (
                  <div
                    key={`${player}-${index}`}
                    className="flex items-center gap-4 rounded-[1.25rem] border border-emerald-100 bg-white/80 px-4 py-3"
                  >
                    <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[linear-gradient(135deg,#dcfce7,#86efac)] text-sm font-bold text-emerald-950">
                      {String(index + 1).padStart(2, "0")}
                    </div>
                    <p className="text-base font-medium text-emerald-950">{player}</p>
                  </div>
                ))}
              </div>
            ) : (
          ...
          <section className="rounded-[2rem] border border-emerald-200/80 bg-[linear-gradient(135deg,rgba(21,128,61,0.95),rgba(52,211,153,0.88))] p-6 text-white shadow-[0_20px_60px_rgba(22,101,52,0.2)] sm:p-8">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-100">อัปเดตล่าสุด</p>
